...
class DA3_Streaming:
    def __init__(self, image_dir, save_dir, config):
        ...
        self.descriptors_dir = os.path.join(save_dir, "descriptors")
        ...
        #Change with your model. They load nested gaint-large 1.1
        ...
        self.skyseg_session = None #Is even used? Probably not

        self.chunk_indices = None  # [(begin_idx, end_idx), ...]

        self.loop_list = []  # e.g. [(1584, 139), ...]

        #Sim(3) optimization. Distances are computed with Lie algebra
        #Because in Sim(3) there's no a maeningful measure of distance.
        # Constrains are relative poses of predictions (chunks?) and ...
        # ... the relative pose (identity) at the loop closure.
        ...
        self.loop_enable = self.config["Model"]["loop_enable"] #When disabling it is a good idea?
    ...
    def load_model(self):
        path = self.config["Weights"]["DA3"] #Must be path to lightning log
        model = DA3Salad.from_lightning_log(path)
    ...
    def process_single_chunk(self, range_1, chunk_idx=None, range_2=None, is_loop=False):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=self.dtype):
                ...
                self.register_descriptors(images, predictions)
